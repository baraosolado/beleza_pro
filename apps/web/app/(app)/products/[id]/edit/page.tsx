...
export default function EditProductPage(): React.ReactElement {
  ...
  useEffect(() => {
    if (product) {
      ...
      setImageDataUrl(product.imageUrl ?? null);
      reset({
        name: product.name ?? '',
        category: product.category ?? '',
        description: product.description ?? '',
        price: product.price ? product.price.toFixed(2).replace('.', ',') : '',
        ...
        <div className="mx-auto w-full max-w-5xl space-y-8">
          ...
          <section className="space-y-2">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Atualize as informações do item no seu estoque.
            </p>
          </section>

          <Card className="mx-auto max-w-3xl overflow-hidden border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
            {productLoading && (
              ...
              >
                ...
                <div className="p-6 sm:p-8">
                  ...
                  <button
                    ...
                    className="group flex w-full flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-200 bg-slate-50/60 p-8 text-center transition-colors hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900/40 dark:hover:bg-slate-900/70"
                  >
                    {imageDataUrl ? (
                      <img
                        src={imageDataUrl}
